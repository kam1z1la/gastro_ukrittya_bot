package com.gastro_ukrittya.bot.handler.reservation.state.edit_reservation.phone_number;
...
@Slf4j
public class PhoneNumberEditStateUpdater implements EditState {
    @Override
    public void handle(AbsSender absSender, Message message, EditContext context) {
        ...
        if (validation.isPhoneNumberCorrect(message.getText())) {
            reservationDto.setPhoneNumber(message.getText());
            editMessage(absSender, message, reservationDto, context);
            nextState(context);
        } else {
            sendMessage(absSender, INCORRECT_PHONE_NUMBER.getError(), message.getChatId());
        }
    }
